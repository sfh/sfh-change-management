...
class Calendar
{
    ...
    */
    function getCalendarLink($text, $month, $year, $uid, $requested_hours)
    {
		return "<a href='javascript:update_calendar(\"$month\", \"$year\", \"$uid\", \"$requested_hours\")'>$text</a>";
    }
    ...
    */
    function getMonthHTML($m, $y, $showYear = 1, $uid, $requested_hours)
    {
        $s = "";
    	...
    	if ($showYear == 1)
    	{
    	    $prevMonth = $this->getCalendarLink("<<", $prev[0], $prev[1], $uid, $requested_hours);
    	    $nextMonth = $this->getCalendarLink(">>", $next[0], $next[1], $uid, $requested_hours);
    	}
    	else
    	{
    	    $prevMonth = "";
    	...
    	$s .= "<tr>\n";
    	$s .= "<td align=\"center\" valign=\"top\">$prevMonth</td>\n";
    	$s .= "<td align=\"center\" valign=\"top\" class=\"calendarHeader\" colspan=\"5\">$header</td>\n"; 
    	$s .= "<td align=\"center\" valign=\"top\">$nextMonth</td>\n";
    	$s .= "</tr>\n";
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
    	        ...
    	        if ($d > 0 && $d <= $daysInMonth)
    	        {
					...
					$s .= $this->changeDateJS($m, $d, $y, $uid, $requested_hours);
    	        }
    	        else
    	        {
    	            $s .= "&nbsp;";
	...
	function changeDateJS($m, $d, $y, $uid, $requested_hours) {

		$current_d = date("d");
		$current_m = date("m");

		if ( $m==$current_m AND $d < $current_d) {
	
			return $d;
		}

		return '<a href="javascript:update_chosen_date(\''.$uid.'\', \''.$requested_hours.'\', \''."$m/$d/$y".'\')">'.$d.'</a>';
	}
